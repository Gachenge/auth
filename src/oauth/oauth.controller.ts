...
export const signupController = async (req: Request, resp: Response) => {
  try {
    ...
    if (error.message === "Email is already registered") {
      ...
      error.message === "Redis client is closed. Cannot perform operations."
    ) {
      return resp
        .status(500)
        .json({ error: "Redis client is closed. Cannot perform operations." });
    } else if (error.message === "Error storing refresh token in Redis") {
      return resp.status(500).json({
        error: `Error storing refresh token in Redis: ${error.message}`,
      });
    }
